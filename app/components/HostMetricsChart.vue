...
const { templates, groupedTemplates } = useTemplates();
...
const appendTemplateEntries = (entries: Array<[string, string]>, source: unknown) => {
  for (const template of ensureArray<any>(source)) {
    if (template.id !== undefined && template.id !== null) {
      entries.push([String(template.id), template.name]);
    }

    for (const variant of ensureArray<any>(template.variants)) {
      entries.push([variant.variant_id, variant.name || template.name]);
      entries.push([variant.id, variant.name || template.name]);
    }
  }
};

const templateNameLookup = computed<Record<string, string>>(() => {
  const entries: Array<[string, string]> = [];

  appendTemplateEntries(entries, templates.value);
  appendTemplateEntries(entries, groupedTemplates.value);

  return entries.reduce<Record<string, string>>((lookup, entry) => {
...
const resolveTemplateName = (templateId: string) => {
  return templateNameLookup.value[templateId] || prettifyTemplateId(templateId);
};
...
const nodeMetrics = computed<MetricRecord>(() => aggregatePayload.value?.node?.metrics ?? {});
const currentMarketMetrics = computed<MetricRecord>(() => currentMarketAggregate.value?.metrics ?? {});
...
const metricOptions = computed<MetricOption[]>(() => {
  const metricNames = getReturnedMetricNames(aggregateData.value as MarketMetricAggregatesResponse | null | undefined);
