 ...
 *
 * Kept free of Vue/Nuxt so it stays easy to reason about and test.
 */
...
export interface JobEventDetail {
  ...
  value: string;
  /** Internal explorer route, when the value has a page here. */
  to?: string;
}
...
> = {
  ...
  Assign: { title: "Assigned to host", tone: "info" },
  Delist: { title: "Delisted from queue", tone: "grey" },
  Work: { title: "Picked up by host", tone: "success" },
  Claim: { title: "Claimed by host", tone: "success" },
  Extend: { title: "Timeout extended", tone: "warning" },
  End: { title: "Stopped by deployer", tone: "warning" },
  Finish: { title: "Finished by host", tone: "success" },
  Complete: { title: "Completed", tone: "success" },
  Quit: { title: "Quit by host", tone: "danger" },
...
const marketDetail = (
  ...
  return {
    ...
    to: `/markets/${event.marketAddress}`,
  };
};
